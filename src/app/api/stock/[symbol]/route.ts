...
export async function POST(
  ...
  try {
    ...
    const stocksBd = await StockModel.find({
      "meta.symbol": symbol,
      "meta.interval": "1month",
    })
      .sort({ "value.datetime": -1 })
      .exec();
    ...
    if (stocksBd.length > 0) {
       LastDateStock = stocksBd[0].value.datetime;
       newDate = parse(LastDateStock, "YYYY-MM-DD", "en-US");
       newDate = addMonth(newDate, 1);
       dateString = format(newDate, "YYYY-MM-DD", "en");
       lastStock = stocksBd[0];
    }
    ...
    if (data.status === "ok") {
      ...
      if (stocksInsert.length > 0) {
        lastStock = stocksInsert[0];
      }
    ...
    if (stockInfo.length == 0) {
      ...
      if (logoUrl.data.status === "error") {
        ...
        urlLogo = logoUrl.data.url
        message = "Logo Insertado";
      }
    } else {
      message = `Ya existe la info del symbol: ${symbol}`;
      urlLogo = stockInfo[0].logo_url;
    }
    return NextResponse.json({
      message: `Insertados: ${registrosInsertados}`,
      lastStock: lastStock,
      logo: message,
      urlLogo,
      Date: getAvailbleDate(),
    });
  } catch (error: any) {
    return NextResponse.json(
